...
export default function AboutusPage() {
    ...
    function Vision2030Card({ category, target }: { category: string; target: string }) {
        return (
            ...
            >
                <div className="flex items-center gap-4 mb-4">
                    <div className="bg-[#146995] p-3 rounded-full w-12 h-12 flex-shrink-0 flex items-center justify-center">
                        {iconMap[category]}
                    </div>
                    <h3 className="text-lg sm:text-xl font-semibold text-[#146995] leading-tight">{category}</h3>
                </div>
                <p className="text-gray-800">{target}</p>
            </div>
        );
    ...
    return (
        <div className="bg-white text-gray-800">
            ...
            {/* ── Header ────────────────────────────────────────────────────────────
                Fix: use `fixed` so the header never sits behind a sidebar overlay,
                and give the logo a stable z-index below the nav/hamburger elements
                but above the page content. The hamburger + nav already handle their
                own z-index internally; we just keep everything on the same stacking
                context here.
            ──────────────────────────────────────────────────────────────────────── */}
            <header className="fixed top-0 left-0 w-full z-40 pointer-events-none">
                <div className="flex items-center justify-between px-4 sm:px-6 pt-3 pb-2">
                    {/* Logo — pointer-events-auto so it remains clickable */}
                    <Link href="/" className="pointer-events-auto relative z-40">
                        <Image
                    ...
                    {/* Mobile hamburger */}
                    <div className="block lg:hidden pointer-events-auto relative z-50">
                        <HamburgerMenu />
                    </div>

                    {/* Desktop nav */}
                    <div className="hidden lg:block pointer-events-auto relative z-40">
                        <NavBar />
                    </div>
                </div>
            </header>
            ...
            {/* ── Banner ──────────────────────────────────────────────────────────── */}
            {/* pt compensates for the fixed header (~100px tall) */}
            <div className="relative pt-[72px] sm:pt-[80px]">
                <Image
                    ...
                    placeholder="blur"
                    blurDataURL="/Aboutus.jpg"
                    className="w-full h-[55vw] min-h-[260px] max-h-[75vh] object-cover"
                />
                <div className="absolute inset-0 bg-black/60 flex items-center px-6 sm:px-10">
                    <div className="text-white max-w-xl">
                        ...
                        <p className="text-base sm:text-lg md:text-xl leading-relaxed">
                            ...
                            enhancing real estate for <strong>People, Planet & Profits</strong>.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <section className="px-6 sm:px-10 py-10">
                <p className="text-base sm:text-lg leading-relaxed max-w-4xl">
                    Founded in 2016 as an extension of the Royal Nest Group—with over 25 years of legacy and 8 million
                    sq. ft. of successfully delivered residential and commercial projects—Ufirm now integrates four
                    synergistic divisions under one roof.
                </p>
            </section>
            ...
            <section className="py-10 px-6 md:px-20 bg-[#e6f3f8]">
                <div className="text-center mb-10">
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-black">Our Divisions</h2>
                    <p className="text-base sm:text-lg max-w-3xl mx-auto mt-4">
                        From sales to service to smart solutions — every UFIRM arm is dedicated to excellence.
                    </p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-10 p-2 md:p-4">
                    <Link href="https://urest.in/">
                        <Feature
                            icon={<FaUsersCog />}
                            title="MAINTAIN – Facility Management"
            ...
            {/* ── Vision 2030 ──────────────────────────────────────────────────── */}
            <section className="py-12 sm:py-16 px-6 md:px-20 bg-white text-black">
                <div className="text-center mb-10">
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-black">{vision2030.title}</h2>
                    <p className="text-base sm:text-lg mt-4 max-w-3xl mx-auto text-gray-700 leading-relaxed">
                        {vision2030.desc}
                    </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-10 p-4 md:p-10">
                    {vision2030.goals.map((goal, index) => (
                        <Vision2030Card
                            key={index}
                            category={goal.category}
            ...
            {/* ── Vision & Mission ─────────────────────────────────────────────── */}
            <section className="py-16 sm:py-20 px-6 md:px-20 bg-gradient-to-br from-[#e6f3f8] via-white to-[#e6f3f8] text-black">
                <div className="text-center mb-12">
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold">Our Vision & Mission</h2>
                    <p className="text-gray-700 text-base sm:text-lg mt-4 font-medium">We deeply care for...</p>
                    <div className="flex justify-center gap-8 mt-4">
                        <div className="flex flex-col items-center">
                            <FaUsers className="text-[#146995] text-3xl sm:text-4xl mb-2" />
                            <span className="text-sm font-semibold text-gray-600">People</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <FaGlobe className="text-green-600 text-3xl sm:text-4xl mb-2" />
                            <span className="text-sm font-semibold text-gray-600">Planet</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <FaCoins className="text-yellow-500 text-3xl sm:text-4xl mb-2" />
                            <span className="text-sm font-semibold text-gray-600">Profits</span>
                        </div>
                    </div>
                </div>
...
function Feature({
    ...
    return (
        <div className="flex flex-col items-center text-center p-6 sm:p-8 bg-white rounded-xl shadow-lg hover:shadow-2xl transition duration-300 border border-gray-100 h-full">
